...
- `npm run postinstall` - Скрипт для очистки кэша из ./node_modules/.cache
- `npm run remove:feature` - Скрипт для вкл/выкл фичей в сборке
...
используется собственный eslint plugin *eslint-plugin-slavio-fsd-plugin*,
который содержит 3 правила: 
1) path-checker - запрещает использовать абсолютные импорты в рамках одного модуля
2) fsd-layer-imports - проверяет корректность использования слоев с точки зрения FSD
...
В папке `scripts` находятся различные скрипты для рефакторинга\упрощения написания кода\генерации отчётов и тд.
...
- [ArticleRecommendsList](/src/features/ArticleRecommendsList/README.md)
